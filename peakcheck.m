...
% Please note that this function is really more of an opportunity for you
% to experiment with your dataset. It is used with Newport_Main so that we
% can observe its effects on individual days. 
% The improved and automated version of this function is peakcheck_auto.m

function [doublePk, avg_RH, std_dev] = peakcheck(RH_peaks, LSP_amps, maxHeight, freqtype, plot2screen)
...
%When plotted below the tracks for all SNR tracks in the LSP, this GMM is useful for identifying double peaks and the significance of each.
...
%the following plotting commands are meant as a check to make sure it is working
%they are to be used with Newport_Main and plot_type == 0
if plot2screen
    %call plotting function for this
    subplot(2,1,2)
    plot_peakcheck(maxHeight, doublePk);
end
...
n1 = doublePk.ComponentProportion(1) * length(RH_peaks); % number of observations in subpopulation 1
n2 = doublePk.ComponentProportion(2) * length(RH_peaks); % number of observations in subpopulation 2

sd1 = sqrt(doublePk.Sigma(:,:,1) *n1 / (n1 - 1)); % standard error of subpop. 1
sd2 = sqrt(doublePk.Sigma(:,:,2) *n2 / (n2 - 1)); % standard error of subpop. 2
df = ( ((sd1^2/n1) + (sd2^2/n2))^2 ) / ( ((sd1^2/n1)^2/(n1-1)) + ((sd2^2/n2)^2/(n2-1))  ); 
% pooled degrees of freedom (because not assuming equal variances)
...
testT = (pk1 - pk2) / sqrt( (sd1^2/n1) + (sd2^2/n2)  ); 
%test t-value, which can be compared with tcrit as well (instead of seeing if conf. interval contains 0)
...
if freqtype ~= 2 
    ...
    elseif max(ci) > 0 && min(ci) < 0
        ...
        std_dev = sqrt(  (n1*(mx1^2) + n2*(mx2^2)) * var(RH_peaks) / ((n1*mx1 + n2*mx2)^2)  );
        return
    end
...
if max(ci) > 0 && min(ci) < 0
    ...
    fprintf('%s\n', 'You have two statistically different peaks in this L2 data. Double-peak values are removed to form an average.');
        ...
        mean2 = (0.2442102134245683/0.1902936) * doublePk.mu(min_ind); %will replace doublePk.mu with L1 mean
...
% n1 = makedist('normal',doublePk.mu(1),sqrt(doublePk.Sigma(1)));
% n2 = makedist('normal',doublePk.mu(2),sqrt(doublePk.Sigma(2)));
% p = doublePk.ComponentProportion;
